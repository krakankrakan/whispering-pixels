...
bool executeCudaKernel(const char* module_file, const char* kernel_name, bool& leaked_data) {
    ...
    //for (unsigned int i = 0; i < 1; i++) {
        ...
        bool cc_seen [1024 * THREADGROUPSIZE];
        for (unsigned int j = 0; j < THREADGROUPS * THREADGROUPSIZE; j++) {
            cc_seen[j] = false;
        }

        for (unsigned int j = 0; j < THREADGROUPS * THREADGROUPSIZE; j+=16) {
            if ((c[j] & 0xFFFF0000) == 0xabcd0000) {
                //covert++;
        ...
        // Covert channel speed
        double covertSizeInGB = (double)(covert * sizeof(int)) / (1024.0 * 1024.0 * 1024.0);
        GB_per_s = (covertSizeInGB) / ((unsigned long)(std::chrono::duration_cast<std::chrono::nanoseconds>(exec_time).count()) / 1e9 );
    ...
    double sizeInGB = (double)(covert_total * sizeof(int)) / (1024.0 * 1024.0 * 1024.0);
